...
# Credential Security Audit — 2026-08-10

**Cron:** `key_inventory` (`0 6 * * *`, daily)  
**Status:** 3/3 cron jobs OK, 5/5 endpoints reachable

## Findings

**No new credential changes since last report (Aug 9).** All env vars, credential files, and rogue key inventory are identical.

### Single status change: Google Sheets API
- **Was:** HTTP 400 on GET `/v4/spreadsheets` ("live, needs Bearer auth")  
- **Now:** HTTP 404 on same endpoint  
- **Interpretation:** Google's API security hardening — unauthenticated GET requests now return 404 instead of 400. The endpoint is still reachable (no timeout). No credential issue; expected behavior for public Google APIs.

### Persistent issues (all unchanged since Aug 01)
| Priority | Issue |
|:--------:|-------|
| RED | Rogue `CUSTOM_API_KEY` (Groq `gsk_kCIQ...`, 56 chars) in process env, not managed by Hermes |
| RED | `DEEPSEEK_API_KEY` in plaintext `.env` |
| RED | OpenRouter key duplicated across 3 locations (`.or_key`, `.or_key_b64`, `~/.hermes/scripts/.or_key_b64`) |
| RED | Google OAuth refresh_token with full Workspace access (Docs/Gmail/Drive/Sheets/Calendar/Tasks/Contacts) |
| YELLOW | OAuth `client_secret` leaked in Telegram chat (Jun 12) |
| GREEN | Invalid stub `credentials.json` in `.hermes/google-workspace/tool_configs/` |

### Session exposures
Zero non-cron exposures. All FTS5 matches are the audit discovering itself.

### Reports saved
- `/opt/data/skills/security/credential-audit/references/audit-2026-08-10.md`
- `/opt/data/security_audit_report_20260810_060000.md`

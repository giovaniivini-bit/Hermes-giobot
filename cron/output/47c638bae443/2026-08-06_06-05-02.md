...
# 🔒 Credential Security Audit — Delta Report (2026-08-06)
...
|-----------|:------:|--------|
| **Env vars** | ✅ No change | 3 active: `CUSTOM_API_KEY` (rogue), `DEEPSEEK_API_KEY` (managed), `HERMES_REDACT_SECRETS` |
| **Credential files** | ✅ No change | 8 active + 7 clean `.env.bak-*` |
| **Rogue keys** | ✅ 1 | `CUSTOM_API_KEY` (`gsk_kCIQ...`, 56 chars) — not in `.env` or `auth.json` |
| **Session exposures** | ✅ No new | All FTS5 hits are cron self-detection only |
| **Cron health** | ✅ 3/3 OK | 0% error rate, 5 runs stable |
| **Endpoints** | ✅ 5/5 live | OpenRouter 401, OAuth 404, Sheets 400, OpenAI 401, DeepSeek 401 |
| **Schedule** | ✅ Throttled | `0 6 * * *` daily — no action needed |

## 🔴 Outstanding Issues (carryover, unchanged)

| Prio | Issue | Since |
|:----:|-------|:-----:|
| 🔴 | `CUSTOM_API_KEY` — rogue Groq key (`gsk_kCIQ...`), unmanaged | Aug 01 |
| 🔴 | `DEEPSEEK_API_KEY` in plaintext `.env` | Aug 01 |
| 🔴 | OpenRouter key in 2 plaintext files + 1 duplicate | Aug 01 |
| 🔴 | Google OAuth refresh_token with full Workspace access (9 scopes) | Aug 01 |
| 🟡 | OAuth `client_secret` (`GOCSPX-a...`) leaked in Telegram Jun 12 | Aug 01 |
| 🟢 | Stub `credentials.json` in `.hermes/tool_configs/` (invalid JSON) | Aug 01 |

## Verification Summary

- **Rogue key cross-ref:** Only `CUSTOM_API_KEY` — same as baseline
- **Base64 dupe check:** `.or_key` == `.or_key_b64` == duplicate → all MATCH
- **`.env.bak-*` files:** All 7 verified — only non-sensitive channel IDs, no keys
- **Session search:** 0 new non-cron exposures
- **Endpoint liveness:** 5/5 reachable (unchanged)

Report saved to: `/opt/data/skills/security/credential-audit/references/audit-2026-08-06--delta.md`

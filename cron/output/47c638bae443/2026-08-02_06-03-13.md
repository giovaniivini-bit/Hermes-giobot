...
# 🔒 Credential Security Audit Report — 2026-08-02

**Generated:** 06:01 UTC | **Schedule:** `0 6 * * *` (daily, throttled from every-minute)  
**Cron:** `key_inventory` (47c638bae443) | **Model:** deepseek/deepseek-v4-flash  
**Reports saved:** `references/audit-2026-08-02.md` + `security_audit_report_20260802_060155.md`  
**Config export:** ✅ Via `mcp_data_hub_secure_export_config`

---

## 🔵 Positive Changes Since Aug 01 Baseline

| Change | Before | After | Status |
|--------|--------|-------|:------:|
| **Log uso 18:00 BRT** | ❌ Sheets API timeout (Jul 31) | ✅ **ok** (Aug 01 21:00) | 🔵 **Resolved** |
| **Schedule** | `* * * * *` (~1,440 runs/day) | `0 6 * * *` (1 run/day) | 🔵 **Throttled** |
| **Endpoint test coverage** | 2 endpoints | 5 endpoints | ✅ **Expanded** |
| **All cron jobs** | 1 of 3 erroring | 3 of 3 **ok** | ✅ **Healthy** |

---

## 🟢 Current State (unchanged since baseline)

### Environment Variables
| Variable | Source | Len | Status |
|----------|--------|:---:|:------:|
| `CUSTOM_API_KEY` | **Process env only** ⚠️ | 56 | ✅ Active (rogue — `gsk_kCIQ...` Groq key) |
| `DEEPSEEK_API_KEY` | `.env` + auth.json | 35 | ✅ Active (`sk-67f...`) |
| `HERMES_REDACT_SECRETS` | Process env | 4 | ✅ System flag |
| DEEPSEEK_BASE_URL | `.env` | 25 | 🟢 Non-cred |

### Credential Files on Disk
8 🔴 files + 7 🟢 clean backups — all unchanged since Jul 02.

### Session Exposures
7 unique non-cron sessions (Jun 12 – Jul 10). No new exposures.  
**Critical:** Google OAuth `client_secret: GOCSPX-...` leaked via Telegram Jun 12.

### Endpoint Liveness (all 5 live)
| Endpoint | Code | Time | Status |
|----------|:----:|:----:|:------:|
| OpenRouter `/auth/key` | **401** | 0.03s | ✅ Live |
| Google OAuth `/token` | **404** | 0.15s | ✅ Live (expects POST) |
| Google Sheets `/spreadsheets` | **400** | 0.24s | ✅ Live (needs auth) |
| DeepSeek `/v1/models` | **401** | 0.46s | ✅ Live |
| OpenAI `/v1/models` | **401** | 0.15s | ✅ Live |
...
| Prio | Issue | Action |
|:----:|-------|--------|
| 🔴 | **`CUSTOM_API_KEY`** rogue key (Groq `gsk_kCIQ...`) — injected externally, not managed by Hermes | Find source (shell profile, container env), add to `.env` or remove |
| 🔴 | **DEEPSEEK_API_KEY** in plaintext `.env` | Rotate, use credential store only |
| 🔴 | **OpenRouter key** in 2 files + duplicate | Rotate `sk-or-...`; delete all `.or_key*` files |
| 🔴 | **Google refresh_token** full Workspace access | Revoke at console.cloud.google.com if over-scoped |
| 🟡 | **OAuth client_secret** leaked in Telegram (Jun 12) | Revoke client; create new one |
| 🟡 | 7 stale `.env.bak-*` + 2 duplicate credential copies | Clean up |
| 🟢 | Empty KEY stubs in `.env` (OPENAI, ANTHROPIC, OPENROUTER) | Remove |
...
| Metric | Count |
|:-------|:-----:|
| Active API keys | 3 (DEEPSEEK, OpenRouter, Groq) |
| Credential files | 8 active + 7 clean backups |
| Rogue keys (env-only) | **1** (`CUSTOM_API_KEY`) |
| Duplicate copies | 3 |
| Exposed sessions | 7 (non-cron) |
| Cron jobs | 3 — **0% error rate** |

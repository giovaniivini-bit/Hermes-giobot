...
**Credential Security Audit — 2026-08-07 06:02 UTC**

| Dimension | Status |
|-----------|:------:|
| Env vars | No change — `CUSTOM_API_KEY` (rogue Groq, 56 chars), `DEEPSEEK_API_KEY` (managed) |
| auth.json | 3 credentials: openrouter, openai-api, deepseek — unchanged |
| Rogue keys | 1: `CUSTOM_API_KEY` (process env only, not in `.env` or `auth.json`) |
| Base64 check | `.or_key` == decoded `.or_key_b64` — MATCH; duplicate at `~/.hermes/scripts/` — MATCH |
| Credential files | 8 active (all valid or documented stubs), 7 clean `.env.bak-*` — all unchanged |
| Session exposures | 0 new non-cron — all FTS5 matches are cron self-detection |
| Endpoints | **5/5 live** — OpenRouter 401, OAuth2 404, Sheets 400, OpenAI 401, DeepSeek 401 |
| Cron health | **3/3 OK** — 0% error rate, 6 consecutive healthy runs |

**No changes since baseline 2026-08-04 (3rd consecutive identical daily run).**

**Reports saved:**
- `references/audit-2026-08-07--delta.md` (durable archive)
- `security_audit_report_20260807_060200.md` (convenience copy)

**Outstanding issues (all carryover from Aug 01):**
- RED: Rogue `CUSTOM_API_KEY` (Groq) in process env
- RED: `DEEPSEEK_API_KEY` in plaintext `.env`
- RED: OpenRouter key in 2 plaintext files + 1 duplicate
- RED: Google OAuth refresh_token with full Workspace access (Gmail/Drive/Sheets/Calendar/Tasks)
- YELLOW: OAuth `client_secret` leaked in Telegram (Jun 12)
- GREEN: Stub `credentials.json` in `.hermes/google-workspace/tool_configs/`

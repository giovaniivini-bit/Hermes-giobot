...
# 🔒 Credential Security Audit — Delta Report (2026-08-05)

**No changes since the 2026-08-04 baseline. All metrics stable for 4 consecutive daily runs.**

## 🟢 State Summary (unchanged)

| Dimension | Status | Detail |
|-----------|:------:|--------|
| **Env vars** | ✅ No change | 3 active: `CUSTOM_API_KEY` (⚠️ rogue), `DEEPSEEK_API_KEY` (managed), `HERMES_REDACT_SECRETS` |
| **Credential files** | ✅ No change | 8 active + 7 clean `.env.bak-*` |
| **Rogue keys** | ⚠️ 1 | `CUSTOM_API_KEY` (`gsk_kCIQ...`, 56 chars) — process env only, not in `.env`/`auth.json` |
| **Session exposures** | ✅ No new | 7 non-cron (Jun 12 – Jul 10), unchanged |
| **Cron health** | ✅ 3/3 OK | 0% error rate |
| **Endpoints** | ✅ 5/5 live | OpenRouter 401, OAuth 404, Sheets 400, OpenAI 401, DeepSeek 401 |
| **Schedule** | ✅ Throttled | `0 6 * * *` (daily) |

## 🔍 What was verified this run

- **Env scan:** Hermes redaction active. `.env` holds `DEEPSEEK_API_KEY`, `DEEPSEEK_BASE_URL`, `TELEGRAM_HOME_CHANNEL`, `TELEGRAM_HOME_CHANNEL_THREAD_ID`. Rogue cross-reference → `CUSTOM_API_KEY` only.
- **auth.json store:** openrouter / openai-api / deepseek (3 managed keys).
- **Base64 integrity:** `.or_key` == decoded `.or_key_b64` (✅ MATCH); duplicate `home/.hermes/scripts/.or_key_b64` == original (✅ MATCH).
- **File integrity:** All key files valid. ⚠️ `/opt/data/.hermes/google-workspace/tool_configs/credentials.json` is **invalid/stub JSON** (carryover — not a live config).
- **Session search:** FTS5 queries (`api_key`, `token OR secret`, `client_secret OR sk-or-`) returned only `cron`-source hits (the audit discovering itself). **Zero new non-cron exposures.**
- **Endpoints:** All 5 reachable (expected codes confirmed — none dead/blocked).

## 🔴 Outstanding Issues (all carryover, unchanged)

| Prio | Issue | Flagged |
|:----:|-------|:-------:|
| 🔴 | `CUSTOM_API_KEY` — rogue Groq key, not managed by Hermes, won't auto-rotate | Aug 01 |
...
- **Credential files (🔴):** 8 · **Duplicates:** 3 · **Clean backups:** 7
- **Rogue keys:** 1 · **Exposed sessions:** 7 · **Cron jobs:** 3 (0% error) · **Endpoints:** 5/5 live

**Reports saved:**
- Archive: `/opt/data/skills/security/credential-audit/references/audit-2026-08-05--delta.md`
- Convenience: `/opt/data/security_audit_report_20260805_060000.md`

**No action required this run.** State is stable and healthy; the daily schedule is appropriately throttled. Outstanding items are long-standing carryovers (notably the rogue `CUSTOM_API_KEY` and the Google refresh_token with broad scope) — rotation is recommended opportunistically but nothing is newly exposed.

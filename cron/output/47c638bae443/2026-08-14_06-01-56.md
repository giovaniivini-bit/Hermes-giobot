...
# Credential Security Audit — 2026-08-14

**Schedule:** `0 6 * * *` (daily, reduced from every minute)  
**Baseline report saved to:** `references/audit-2026-08-14.md`

## Summary

| Area | Findings |
|------|----------|
| **Env vars with keys** | 3 in process env (`CUSTOM_API_KEY`, `HERMES_SESSION_KEY`, `HERMES_REDACT_SECRETS`) |
| **.env keys** | 4 entries: DEEPSEEK_API_KEY, DEEPSEEK_BASE_URL, TELEGRAM_HOME_CHANNEL, empty THREAD_ID |
| **auth.json credentials** | 3 registered: OpenRouter API, OpenAI API, DeepSeek API |
| **Rogue keys** | 2 found: `CUSTOM_API_KEY` (Groq, gsk_kCIQ...) and `HERMES_SESSION_KEY` — not managed by Hermes |
| **Credential files** | 8 on disk; 3 copies of OpenRouter key (1 plaintext + 2 base64 dupes); 1 corrupt `credentials.json` |
| **Session exposures** | 0 non-cron credentials exposed in chat history |
| **Cron jobs** | All 3 running OK |
| **Endpoints** | 5 tested — all reachable. Sheets API returned 404 instead of usual 400 (may indicate routing change) |

## Priority Actions

🔴 **Remove `.or_key` duplicate copies** — reduce blast radius across 3 locations  
🔴 **Fix corrupt** `/opt/data/.hermes/google-workspace/tool_configs/credentials.json` — re-run Google OAuth flow  
🟡 **Investigate Sheets API 404** — verify dependent cron scripts still work  
🟡 **Identify source of** `CUSTOM_API_KEY` — rogue Groq key injected externally

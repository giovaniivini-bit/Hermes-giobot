...
This is the first full baseline after the schedule was throttled from every-minute (`* * * * *`) to daily (`0 6 * * *`). The environment state is stable — no new exposures or changes since yesterday's report.

**Full report saved to:** `/opt/data/skills/security/credential-audit/references/audit-2026-08-08.md`

## Key Findings

| Area | Status |
|------|--------|
| **Managed keys** | 3 active (OpenRouter, OpenAI, DeepSeek) — all via `auth.json` |
| **Rogue key** | `CUSTOM_API_KEY` (Groq `gsk_...`, 56 chars) still in process env only — flagged Aug 01, unresolved for 8 days |
| **Credential files** | 8 🔴 risk, 2 🟡, 1 🟢 (7 clean `.env.bak-*` files) |
| **Duplicates** | OpenRouter key in 3 locations; Google client secret duplicated in 2 files |
| **Endpoints** | All 5 live (OpenRouter 401, OAuth 404, Sheets 404, OpenAI 401, DeepSeek 401) |
| **Cron health** | 3/3 OK, 0% error rate |
| **Session exposure** | Zero non-cron exposures detected |

## Outstanding Issues (all carryover)

The most actionable item remains the **rogue `CUSTOM_API_KEY`** — a Groq API key injected externally into the process environment outside Hermes' credential management. If this is not needed, it should be removed from whatever injector (shell profile, container env, Docker `-e`) placed it there.

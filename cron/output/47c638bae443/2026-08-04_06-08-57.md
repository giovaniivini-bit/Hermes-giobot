...
# 🔒 Credential Security Audit — 2026-08-04 06:33 UTC

**Delta vs 2026-08-03 baseline: No changes detected.**

| Phase | Status |
|:------|:------:|
| ✅ Phase 1 — Environment variables, .env, auth.json, rogue keys | Complete |
| ✅ Phase 2 — Session history (3 FTS5 queries) | Complete |
| ✅ Phase 3 — Filesystem scan (maxdepth 5, duplicate checks) | Complete |
| ✅ Phase 4.5 — Endpoint liveness (all 5 tested) | Complete |
| ✅ Phase 5 — Cron job audit (3 jobs, credential flow traced) | Complete |
| ✅ Phase 6 — Reports saved | Complete |

---

## 🟢 All Stable — 3rd Consecutive Identical Daily Run

| Dimension | Status |
|-----------|:------:|
| **Env vars** | ✅ 3 active: `CUSTOM_API_KEY` (rogue Groq, 56 chars), `DEEPSEEK_API_KEY`, `HERMES_REDACT_SECRETS` |
| **Credential files** | ✅ 8 active + 7 clean backups — all SHA256 unchanged since Jul 02 |
| **Rogue keys** | ✅ 1 — `CUSTOM_API_KEY` (`gsk_kCIQ...`, process-env only, not in `.env` or `auth.json`) |
| **Session exposures** | ✅ 7 tracked (Jun 12 – Jul 10) — no new non-cron exposures |
| **Cron health** | ✅ 3/3 **OK** — 0% error rate. Sheets timeout self-resolved Aug 01, stayed OK |
| **Endpoints** | ✅ 5/5 live — OpenRouter 401, OAuth 404, Sheets 400, OpenAI 401, DeepSeek 401 |
| **Schedule** | ✅ `0 6 * * *` (daily) — throttled from `* * * * *` on Aug 01 |

## 🔴 Outstanding Issues (all carryover)

| Prio | Issue |
|:----:|-------|
| 🔴 | `CUSTOM_API_KEY` — rogue Groq key in process env, not managed by Hermes |
| 🔴 | `DEEPSEEK_API_KEY` in plaintext `.env` |
| 🔴 | OpenRouter key in 2 plaintext files + 1 duplicate (`~/.hermes/scripts/.or_key_b64`) |
| 🔴 | Google OAuth refresh_token — full Gmail/Drive/Sheets/Calendar/Tasks access |
| 🟡 | OAuth `client_secret` (`GOCSPX-...`) leaked in Telegram (Jun 12) |
| 🟡 | 7 stale `.env.bak-*` files |
| 🟢 | Empty KEY stubs in `.env` (OPENAI, ANTHROPIC, OPENROUTER) |

## Reports Saved
- **Durable archive:** `skills/security/credential-audit/references/audit-2026-08-04--delta.md`
- **Convenience copy:** `security_audit_report_20260804_063300.md`

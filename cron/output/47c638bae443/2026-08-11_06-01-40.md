...
**Credential Security Audit — 2026-08-11**

## Summary: No significant findings. Environment stable.

| Metric | Value |
|--------|-------|
| **Endpoints verified** | 5/5 reachable (OpenRouter 401, OAuth 404, Sheets 400, OpenAI 401, DeepSeek 401) |
| **Active API keys** | 3 (DEEPSEEK, OpenRouter, Groq rogue key in process env) |
| **Rogue keys detected** | `CUSTOM_API_KEY` (Groq, 56 chars) — unchanged since Aug 01 |
| **Session exposures** | Zero non-cron source exposures across all FTS5 queries |
| **Cron health** | 3/3 OK, 0% error rate |
| **Schedule** | Daily (`0 6 * * *`), throttled since Aug 01 |

### Minor changes since yesterday (Aug 10):
- **Google Sheets API:** HTTP status shifted from 404 to 400 — still means "GET requires Bearer auth", just different Google API response format. Not a credential issue.
- **`.hermes/google-workspace/tool_configs/credentials.json`:** Now 345B (previously zero bytes). Content is markdown-wrapped JSON — still unusable for programmatic auth. No risk increase.

### Persistent issues (unchanged since Aug 01 baseline):
1. 🟥 Rogue `CUSTOM_API_KEY` (Groq key injected via process env, unmanaged by Hermes)
2. 🟥 `DEEPSEEK_API_KEY` stored as plaintext in `.env` instead of credential store
3. 🟥 OpenRouter key duplicated across 3 locations (all validated matching)
4. 🟥 Google OAuth refresh_token with full Workspace access
5. 🟨 OAuth client_secret exposed in Telegram chat history (Jun 12)
6. 🟩 7 stale `.env.bak-*` files (non-sensitive, cleanup recommended)

Reports saved to:
- `/opt/data/skills/security/credential-audit/references/audit-2026-08-11.md`
- `/opt/data/security_audit_report_20260811_060000.md`

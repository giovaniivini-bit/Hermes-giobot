...
**Credential Security Audit — 2026-08-12**

Report saved to `references/audit-2026-08-12--delta.md`

**Summary: Stable environment, minor changes detected.**

| Area | Status | Detail |
|------|--------|--------|
| **Env vars** | No change | Same 4 `.env` keys, 3 rogue env-only keys (`CUSTOM_API_KEY` Groq, `HERMES_REDACT_SECRETS`, `HERMES_SESSION_KEY`) |
| **Credential files** | +2 new files | `auth.lock` (empty), `dashboard-auth.log` (2 failed login attempts from foreign IPs) |
| **Session exposures** | None | All FTS5 hits are cron self-detection; zero non-cron source exposures |
| **Cron health** | 3/3 OK | All jobs running without errors |
| **Endpoints** | 5/5 reachable | Sheets API shifted HTTP 400 → 404 (still requires auth, not a credential issue) |

**New finding (Yellow):** `/opt/data/logs/dashboard-auth.log` shows 2 failed basic auth login attempts from IPs `123.100.137.18` and `131.143.214.240`. Not Hermes-managed — may indicate an external service under brute-force. Recommend verifying whether this dashboard log belongs to any known service on this host.

**Unchanged carryovers:** OpenRouter key in 3 locations, DeepSeek key in plaintext `.env`, Google OAuth refresh token with full Workspace access, stale client_secret exposure in Telegram chat, 7 empty `.env.bak-*` files, invalid markdown-wrapped credentials.json.

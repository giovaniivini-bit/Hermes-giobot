...
class ComfyRunner:
    ...
    def monitor_ws(self, pid: str, *, timeout: float = 300.0,
        ...
        try:
            ...
            return self.poll_status(pid, timeout=timeout)
        ...
        try:
            ...
            while time.time() < deadline:
                ...
                job_id = mdata.get("prompt_id")
                if job_id is not None and job_id != pid:
                    continue
    ...
    # ---------- outputs ----------
    def get_outputs(self, pid: str) -> dict:
        if self.is_cloud:
            # Try /jobs/{id} first (returns full job with outputs); fall back to /history_v2
            r = http_get(self._url(f"/jobs/{pid}"), headers=self.headers, retries=2)
            if r.status == 200:
                try:
                    ...
                    pass
            # Fallback
            r = http_get(self._url(f"/history/{pid}"), headers=self.headers, retries=2)
            if r.status == 200:
                try:
                    body = r.json() or {}
                except Exception:
                    body = {}
                if isinstance(b, dict) and pid in b:
                    return b[pid].get("outputs", {}) or {}
                if isinstance(b, dict) and "outputs" in b:
                    return b["outputs"] or {}
            return {}
        # Local
        r = http_get(self._url(f"/history/{pid}"), headers=self.headers, retries=2)
    ...
    # ---------- queue / cancel ----------
    def cancel(self, pid: str | None = None) -> bool:
        if pid:
            r = http_post(
                self._url("/queue"), headers=self.headers,
                json_body={"delete": [pid]}, retries=1,
            )
            return r.status == 200
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.submit_only:
        emit_json({"status": "submitted", "prompt_id": pid, "warnings": warnings})
        return 0
    ...
    log(f"Submitted: pid={pid}, waiting (timeout={timeout}s)…")
    ...
    try:
        if args.ws:
            wait_result = runner.monitor_ws(pid, timeout=timeout, on_progress=_on_progress)
        else:
            wait_result = runner.poll_status(pid, timeout=timeout)
    except KeyboardInterrupt:
        log(f"Interrupted — cancelling job {pid} on server…")
        try:
            runner.cancel(pid)
        except Exception as e:
    ...
    if wait_result["status"] == "error":
        emit_json({"status": "error", "pid": pid, "details": wait_result.get("data")})
        return 1
    if wait_result["status"] == "cancelled":
        emit_json({"status": "cancelled", "pid": pid})
        return 1

    # ---- Outputs ----
    outputs = wait_result.get("outputs")
    if not outputs:
        outputs = runner.get_outputs(pid)

    if args.no_download:
        emit_json({
            "status": "success", "pid": pid,
